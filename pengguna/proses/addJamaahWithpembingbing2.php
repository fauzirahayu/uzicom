...
if (isset($_POST['simpan'])) {
    ...
    $sqlPembimbing = "SELECT id FROM pembimbing_haji WHERE (SELECT COUNT(*) FROM jamaah_2027 WHERE id_pembimbing = pembimbing_haji.id) < 20 ORDER BY RAND() LIMIT 1";
    ...
    if ($resultPembimbing->num_rows > 0) {
        ...
        echo '<script>alert("Tidak ada pembimbing yang tersedia. Semua pembimbing sudah mencapai batas maksimal."); window.location.href = "../daftar_jamaah2.php";</script>';
        $conn->close();
        exit();
    }
    ...
    if ($row['total'] >= 50) {
        echo '<script>alert("Jumlah jamaah haji sudah mencapai batas maksimal 50 orang."); window.location.href = "../jamaah2027.php";</script>';
        exit;
    }
    ...
    if ($totalNIK > 0) {
        echo '<script>alert("NIK sudah terdaftar di tahun lain. Silakan gunakan NIK yang berbeda."); window.location.href = "../daftar_jamaah2.php";</script>';
        $conn->close();
        exit();
    }
    ...
    if ($stmt->execute()) {
        header('Location: ../jamaah/jamaah2.php');
        exit;
    } else {
