...
if (isset($_POST['simpan'])) {
    ...
    if (empty($id_pembimbing) || empty($nama_lengkap) || empty($nik) || empty($jenis_kelamin) || empty($tanggal_lahir) || empty($alamat) || empty($telepon) || empty($no_paspor)) {
        echo "<script type='text/javascript'>alert('Semua field harus diisi!');window.location.href='../../contern/jamaahHaji/tambahJamaah2029.php';</script>";
        exit();
    }

    // Validasi NIK dan Telepon tidak boleh negatif
    if ($nik < 0 || $telepon < 0) {
        echo "<script type='text/javascript'>alert('NIK dan Nomor Telepon tidak boleh negatif!');window.location.href='../../contern/jamaahHaji/tambahJamaah2029.php';</script>";
        exit();
    }
    ...
    if ($totalNik > 0) {
        echo '<script>alert("NIK sudah terdaftar. Silakan gunakan NIK yang berbeda."); window.location.href = "../../contern/jamaahHaji/tambahJamaah2029.php";</script>';
        $conn->close();
        exit();
    }
    ...
    if ($no_porsi != '' && $no_porsi != '-') {
        ...
        if ($totalNoPorsi > 0) {
            echo '<script>alert("No Porsi sudah terdaftar di seluruh tahun. Silakan gunakan No Porsi yang berbeda."); window.location.href = "../../contern/jamaahHaji/tambahJamaah2029.php";</script>';
            $conn->close();
            exit();
        }
    }
