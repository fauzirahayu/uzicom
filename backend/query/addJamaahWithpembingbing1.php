...
if (isset($_POST['simpan'])) {
    ...
    $cekJumlah = $conn->prepare("SELECT COUNT(*) FROM jamaah_2027 WHERE id_pembimbing = ?");
    ...
    $cekJumlah->close();
    if ($jumlahJamaah >= 20) {
        echo '<script>alert("Pembimbing ini sudah membimbing 20 jamaah. Silakan pilih pembimbing lain."); window.location.href = "../../contern/jamaahHaji/tambahJamaah2027.php";</script>';
        $conn->close();
        exit();
    }
        // Cek jumlah jamaah haji
    $result = $conn->query("SELECT COUNT(*) as total FROM jamaah_2027");
    $row = $result->fetch_assoc();
    if ($row['total'] >= 70) {
        echo '<script>alert("Jumlah jamaah haji sudah mencapai batas maksimal 50 orang."); window.location.href = "../../contern/jamaahHaji/jamaah2027.php";</script>';
        exit;
    }
    ...
    if ($totalNIK > 0) {
        echo '<script>alert("NIK sudah terdaftar di tahun lain. Silakan gunakan NIK yang berbeda."); window.location.href = "../../contern/jamaahHaji/tambahJamaah2027.php";</script>';
        $conn->close();
        exit();
    }
    ...
    if ($no_porsi == $nik) {
        echo '<script>alert("No Porsi tidak boleh sama dengan NIK. Silakan gunakan No Porsi yang berbeda."); window.location.href = "../../contern/jamaahHaji/tambahJamaah2027.php";</script>';
        $conn->close();
        exit();
    }
    ...
    if ($totalNoPorsi > 0) {
        echo '<script>alert("No Porsi sudah terdaftar di tahun lain. Silakan gunakan No Porsi yang berbeda."); window.location.href = "../../contern/jamaahHaji/tambahJamaah2027.php";</script>';
        $conn->close();
        exit();
    }
    ...
    $stmt = $conn->prepare("INSERT INTO jamaah_2027 (
