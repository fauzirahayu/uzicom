...
if (isset($_POST['update'])) {
    ...
    if ($nik < 0 || $telepon < 0) {
        echo "<script type='text/javascript'>alert('NIK dan Nomor Telepon tidak boleh negatif!');window.location.href='../../contern/jamaahHaji/editJamaah2027.php?id=" . $id . "';</script>";
        exit();
    }
    ...
    if ($totalNik > 0) {
        echo '<script>alert("NIK sudah terdaftar. Silakan gunakan NIK yang berbeda."); window.location.href = "../../contern/jamaahHaji/editJamaah2027.php?id=' . $id . '";</script>';
        $conn->close();
        exit();
    }
    ...
    if ($no_porsi != '' && $no_porsi != '-') {
        ...
        if ($totalNoPorsi > 0) {
            echo '<script>alert("No Porsi sudah terdaftar di seluruh tahun. Silakan gunakan No Porsi yang berbeda."); window.location.href = "../../contern/jamaahHaji/editJamaah2027.php?id=' . $id . '";</script>';
            $conn->close();
            exit();
        }
    }
    ...
    if ($status === 'lunas') {
        ...
        if ($no_porsi == $nik) {
            echo '<script>alert("No Porsi tidak boleh sama dengan NIK. Silakan gunakan No Porsi yang berbeda."); window.location.href = "../../contern/jamaahHaji/editJamaah2027.php?id=' . $id . '";</script>';
            $conn->close();
            exit();
        }
    }
