...
if (isset($_POST['update'])) {
    ...
    if (empty($id_pembimbing) || empty($nama_lengkap) || empty($nik) || empty($jenis_kelamin) || empty($tanggal_lahir) || empty($alamat) || empty($telepon) || empty($no_paspor)) {
        echo "<script type='text/javascript'>alert('Semua field harus diisi!');window.location.href='../../contern/jamaahHaji/editJamaah2029.php?id=" . $id . "';</script>";
        exit();
    }

    // Validasi NIK dan Telepon tidak boleh negatif
    if ($nik < 0 || $telepon < 0) {
        echo "<script type='text/javascript'>alert('NIK dan Nomor Telepon tidak boleh negatif!');window.location.href='../../contern/jamaahHaji/editJamaah2029.php?id=" . $id . "';</script>";
        exit();
    }
    ...
    if ($totalNik > 0) {
        echo '<script>alert("NIK sudah terdaftar. Silakan gunakan NIK yang berbeda."); window.location.href = "../../contern/jamaahHaji/editJamaah2029.php?id=' . $id . '";</script>';
        $conn->close();
        exit();
    }
    ...
    $cekJumlah = $conn->prepare("SELECT COUNT(*) as total FROM (
        ...
        UNION ALL
        SELECT id FROM jamaah_2029 WHERE id_pembimbing = ? AND id != ?
    ) as all_jamaah");
    $cekJumlah->bind_param("iiiii", $id_pembimbing, $id_pembimbing, $id_pembimbing, $id_pembimbing, $id);
    $cekJumlah->execute();
    $cekJumlah->bind_result($jumlahJamaah);
    $cekJumlah->fetch();
    $cekJumlah->close();
    if ($jumlahJamaah >= 20) {
        echo '<script>alert("Pembimbing ini sudah membimbing 20 jamaah. Silakan pilih pembimbing lain."); window.location.href = "../../contern/jamaahHaji/editJamaah2029.php?id=' . $id . '";</script>';
        $conn->close();
        exit();
    } else {
    ...
    if ($no_porsi != '' && $no_porsi != '-') {
        ...
        if ($totalNoPorsi > 0) {
            echo '<script>alert("No Porsi sudah terdaftar di seluruh tahun. Silakan gunakan No Porsi yang berbeda."); window.location.href = "../../contern/jamaahHaji/editJamaah2029.php?id=' . $id . '";</script>';
            $conn->close();
            exit();
        }
    }
    ...
    if ($status === 'lunas') {
        ...
        if ($no_porsi == $nik) {
            echo '<script>alert("No Porsi tidak boleh sama dengan NIK. Silakan gunakan No Porsi yang berbeda."); window.location.href = "../../contern/jamaahHaji/editJamaah2029.php?id=' . $id . '";</script>';
            $conn->close();
            exit();
        }
    }
